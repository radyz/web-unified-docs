...
For a Vault Enterprise deployment, pass the license secret at install time — the chart will automatically use the enterprise-compatible CSI provider image:
...
-> **Enterprise image auto-selection:** If you are deploying against a Vault Enterprise cluster and have set [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the Vault Secrets Store CSI provider automatically uses the enterprise-compatible image. You do not need to set any additional image values. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for details.
