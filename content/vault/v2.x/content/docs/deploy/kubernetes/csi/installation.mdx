...
last_modified: 2026-09-21T09:55:21.000Z
...
For a Vault Enterprise deployment, pass the license secret at install time — the chart will automatically use the enterprise-compatible image for the Vault Agent sidecar the CSI provider uses:
...
-> **Enterprise image auto-selection:** If you are deploying against a Vault Enterprise cluster and have set [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the CSI provider's Vault Agent sidecar (`csi.agent.image`, used when `csi.agent.enabled` is `true`) automatically uses the enterprise-compatible image. The CSI provider's own image (`csi.image`) is unaffected. You do not need to set any additional image values. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for details.
...
- OpenShift 4.14+
- You must have the [Secrets Store CSI driver operator](https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/storage/using-container-storage-interface-csi#persistent-storage-csi-secrets-store-driver-install_persistent-storage-csi-secrets-store)
  from Red Hat installed.
