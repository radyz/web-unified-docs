...
- `injector` - Values that configure running a Vault Agent Injector Admission Webhook Controller within Kubernetes.
  ...
  - `agentImage` - Values that configure the Vault Agent sidecar image that the injector adds to annotated application pods.
    ...
    - `repository` (`string: "hashicorp/vault"`) - The name of the Docker image for the Vault Agent sidecar. This should be set to the official Vault Docker image. When [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) is set and this field is left at its default, the chart resolves it to `hashicorp/vault-enterprise` instead. Refer to [Automatic image selection](/vault/docs/platform/k8s/helm/enterprise#automatic-image-selection) for details.

    - `tag` (`string: ""`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar. **Vault 1.3.1+ is required by the admission controller**. When empty, the chart resolves this to the chart's `AppVersion`, or `AppVersion-ent` when an Enterprise license is configured.
...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    - `repository` (`string: "hashicorp/vault"`) - The name of the Docker image for the containers running Vault. When [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) is set and this field is left at its default, the chart resolves it to `hashicorp/vault-enterprise` instead. Refer to [Automatic image selection](/vault/docs/platform/k8s/helm/enterprise#automatic-image-selection) for details.
    ...
    - `tag` (`string: ""`) - The tag of the Docker image for the containers
      ...
      to the chart's `AppVersion`, or `AppVersion-ent` when an Enterprise license
      is configured.
...
- `csi` - Values that configure running the Vault CSI Provider.
  ...
  - `agent` - Configures the Vault Agent sidecar for the CSI Provider
    ...
    - `image` - Values that configure the Vault Agent sidecar image for the CSI Provider.
      ...
      - `tag` (`string: ""`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar. When empty, the chart resolves this to the chart's `AppVersion`, or `AppVersion-ent` when an Enterprise license is configured.
