...
last_modified: 2026-08-31T05:51:42.000Z
...
-> **Enterprise image auto-selection:** If you provide a Vault Enterprise license via [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the chart automatically uses the `hashicorp/vault-enterprise` image for the Vault server and for the Vault Agent sidecar images injected by the Agent Injector and used by the CSI Provider. The Agent Injector's and CSI Provider's own images are unaffected. You no longer need to manually set `server.image.repository` or `server.image.tag`. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for full details.
