...
-> **Enterprise image auto-selection:** If you provide a Vault Enterprise license via [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the chart automatically uses the `hashicorp/vault-enterprise` image for the Vault server and enterprise-compatible images for the Vault Agent Injector and Vault CSI Provider. You no longer need to manually set `server.image.repository`. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for full details.
