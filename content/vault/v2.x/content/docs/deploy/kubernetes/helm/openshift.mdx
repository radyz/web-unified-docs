...
last_modified: 2026-08-31T05:51:43.000Z
...
If you have a Vault Enterprise license, export it and create a Kubernetes secret before installing:
...
Then install Vault Enterprise on OpenShift. When `server.enterpriseLicense.secretName` is set, the chart automatically uses `hashicorp/vault-enterprise` for the Vault server and enterprise-compatible images for the Vault Agent Injector and Vault CSI Provider — no manual image configuration required:
...
Additional parameters (HA, Raft, dev mode, etc.) can be appended as `--set` flags or via a values file as needed. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for full details including pinning a specific release tag or using a custom registry.
