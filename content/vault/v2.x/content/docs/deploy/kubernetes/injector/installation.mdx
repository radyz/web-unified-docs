...
last_modified: 2026-09-21T09:55:23.000Z
...
For a Vault Enterprise deployment, pass the license secret at install time — the chart will automatically use the enterprise-compatible image for the injected Vault Agent sidecar:
...
-> **Enterprise image auto-selection:** If you are deploying against a Vault Enterprise cluster and have set [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the Vault Agent sidecar injected into your application pods (`injector.agentImage`) automatically uses the enterprise-compatible image. The Agent Injector's own image (`injector.image`) is unaffected. You do not need to set any additional image values. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for details.
